...
class gluster::repo::apt (
  ...
  # basic sanity check
  if $version == 'LATEST' and $::operatingsystem != 'Ubuntu' {
    $repo_ver = $version
  } else {
    if $version =~ /^\d\.\d$/ {
  ...
  # the Gluster repo only supports x86_64 and i386. The Ubuntu PPA also supports armhf and arm64.
  case $::operatingsystem {
    'Debian': {
      ...
      $repo_url = "${repo_base}/${repo_ver}/Debian/${::lsbdistcodename}/apt/"
    }
    default: {
      $arch = $::architecture ? {
        'amd64'      => 'amd64',
      ...
      $repo_url = "http://ppa.launchpad.net/gluster/glusterfs-${repo_ver}/ubuntu/"   
    }
  }
  if ! $arch {
